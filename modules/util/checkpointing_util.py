...
def _checkpoint_layer_patterns(config: TrainConfig) -> list[str]:
    return [pattern.strip() for pattern in config.gradient_checkpointing_layers if pattern.strip()]


def should_checkpoint_layer(layer_name: str, patterns: list[str]) -> bool:
    return not patterns or any(pattern in layer_name for pattern in patterns)
...
def _create_checkpoints_for_module_list(
    ...
    layer_index: int,
    compile: bool,
    module_list_name: str | None,
    layer_name_patterns: list[str],
) -> int:

    for module_index, layer in enumerate(module_list):
        layer_name = f"{module_list_name}.{module_index}" if module_list_name else str(module_index)
        if not should_checkpoint_layer(layer_name, layer_name_patterns):
            continue

        if isinstance(module_list[module_index], BaseCheckpointLayer):
            continue
        module_list[module_index] = create_checkpoint(
            layer,
            train_device,
            include_from_offload_param_names,
            conductor,
...
def enable_checkpointing(
    ...
    layer_name_patterns = _checkpoint_layer_patterns(config)
    module_list_names = {
        id(module): module_name for module_name, module in model.named_modules() if isinstance(module, nn.ModuleList)
    }
    ...
    for type_or_list, param_names in lists:
        ...
        if isinstance(type_or_list, nn.ModuleList):
            ...
            layer_index = _create_checkpoints_for_module_list(
                ...
                layer_index,
                compile=compile,
                module_list_name=module_list_names.get(id(module_list)),
                layer_name_patterns=layer_name_patterns,
            )
        else:
            module_type = type_or_list
            for module_list_name, child_module in model.named_modules():
                if (
                    isinstance(child_module, nn.ModuleList)
                    and len(child_module) > 0
                    and isinstance(child_module[0], module_type)
                ):
                    module_list = child_module
                    assert all(isinstance(module, module_type) for module in child_module)
                    layer_index = _create_checkpoints_for_module_list(
                        module_list,
                        param_names,
                        conductor if offload_enabled else None,
                        ...
                        layer_index,
                        compile=compile,
                        module_list_name=module_list_name,
                        layer_name_patterns=layer_name_patterns,
                    )

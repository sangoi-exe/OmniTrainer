...
class TrainConfig(BaseConfig):
    ...
    lora_modules_alpha_rules: list[dict[str, int | float]]
    lora_key_export_path: str
    aspect_bucketing_quantization_override: int
    ...
    # Sangoi TrainGPS/DataRecorder
    train_gps_save_it: bool
    train_gps_use_it: bool
    train_gps_path: str
    train_gps_weight: float
    train_gps_penalty_metric: TrainGPSPenaltyMetric
    data_recorder: bool
    run_number: int
    ...
    def default_values() -> "TrainConfig":
        ...
        data.append(("lora_modules_alpha_rules", [], list[dict[str, int | float]], False))
        data.append(("lora_layers_blacklist", [], list[str], False))
        data.append(("lora_key_export_path", "", str, False))
        data.append(("aspect_bucketing_quantization_override", 0, int, False))
        ...
        data.append(("priority_lr", 1.0, float, False))
        data.append(("priority_beta", 0.9, float, False))
        data.append(("priority_radius", 0, int, False))
        ...
        data.append(("train_gps_penalty_metric", TrainGPSPenaltyMetric.MSE, TrainGPSPenaltyMetric, False))

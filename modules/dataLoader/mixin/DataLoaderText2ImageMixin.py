...
class DataLoaderText2ImageMixin(metaclass=ABCMeta):
    ...
    def _aspect_bucketing_in(
        self, config: TrainConfig, _aspect_bucketing_quantization: int, frame_dim_enabled: bool = False
    ):
        aspect_bucketing_quantization = config.aspect_bucketing_quantization_override
        if aspect_bucketing_quantization < 0:
            raise ValueError("aspect_bucketing_quantization_override must be greater than or equal to 0")
        if aspect_bucketing_quantization == 0:
            aspect_bucketing_quantization = _aspect_bucketing_quantization

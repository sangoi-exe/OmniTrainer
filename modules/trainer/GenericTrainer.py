...
class GenericTrainer(BaseTrainer):
    ...
    def __handle_pause_logic(self):
        if not self.is_paused:
            return

        self.callbacks.on_update_status("Pausing... Moving model to CPU")
        logFun("[Pause] Moving model to temporary device.", lvl="info")
        try:
            self.model.to(self.temp_device)
            self.model.eval()
            torch_gc()
            self.callbacks.on_update_status(f"Paused. Model on {self.temp_device}.")
            ...
            if not self.commands.get_stop_command():
                self.callbacks.on_update_status("Resuming... Moving model to training device")
                self.model_setup.setup_train_device(self.model, self.config)
                torch_gc()
            ...
            self.is_paused = False
            traceback.print_exc()
            logFun("[Pause] Failed while pausing or resuming training.", lvl="error")
            self.callbacks.on_update_status("Error during pause/resume.")
            self.commands.stop()

    def __grad_norms_by_group(self) -> dict[str, float]:
        ...
        for param_group in self.model.optimizer.param_groups:
            ...
            for parameter in param_group["params"]:
                ...
                norm_squared += grad_norm.pow(2).item()

            if norm_squared > 0.0:
                grad_norms[group_name] = norm_squared ** 0.5

        return grad_norms

    @staticmethod
    def __metric_as_float(value) -> float | None:
        if value is None:
            return None
    ...
    def __record_prodigy_step(self, train_progress: TrainProgress, grad_norms_by_group: dict[str, float]):
        if self.recorder is None or not hasattr(self.model.optimizer, "pop_stats"):
            return

        self.recorder.set_current_step(train_progress.global_step)
        for stat_data in self.model.optimizer.pop_stats():
            group_name = stat_data.get("name")
            if group_name is None:
                continue

            self.recorder.log_metrics_step(
                name=group_name,
                d_num_pdgy=self.__metric_as_float(stat_data.get("d_num")),
                d_den_pdgy=self.__metric_as_float(stat_data.get("d_den")),
                dlr_pdgy=self.__metric_as_float(stat_data.get("dlr")),
                grad_norm=grad_norms_by_group.get(group_name),
            )

    def __save_sangoi_outputs(self, save_path: str):
        ...
        if self.config.train_gps_save_it and train_gps is not None:
            output_dir = os.path.join(self.config.workspace_dir, "training_deltas")
            os.makedirs(output_dir, exist_ok=True)
            delta_filename = f"{model_name}_Deltas_Run{self.config.run_number}_{timestamp}.json"
            delta_save_path = os.path.join(output_dir, delta_filename)
            logFun(f"[TrainGPS] Saving deltas to {delta_save_path}", lvl="info")
            train_gps.save_log_to_file(delta_save_path)

        if self.recorder is not None:
            output_dir = os.path.join(self.config.workspace_dir, "data_recorder")
            os.makedirs(output_dir, exist_ok=True)
            profile_filename = f"{model_name}_Profile_Run{self.config.run_number}_{timestamp}.json.gz"
            profile_save_path = os.path.join(output_dir, profile_filename)
            logFun(f"[DataRecorder] Saving profile to {profile_save_path}", lvl="info")
    ...
    def train(self):
        ...
        for _epoch in tqdm(epochs, desc="epoch") if multi.is_master() else epochs:
            ...
            for batch in batches:
                ...
                with (
                    ...
                    if self.config.timestep_distribution == TimestepDistribution.PRIORITY_SAMPLING:
                        self.model_setup.update_sampler_priorities(
                            model_output_data["timestep"], loss.detach(), self.config
                        )
                    ...
                    if self.__is_update_step(train_progress):
                        ...
                        grad_norms_by_group = (
                            self.__grad_norms_by_group()
                            if self.recorder is not None and multi.is_master()
                            else {}
                        )
            ...
            train_gps: TrainGPS | None = getattr(self.model, "train_gps", None)
            if train_gps is not None and self.config.train_gps_save_it:
                train_gps.log_epoch_deltas(train_progress.epoch - 1)
    ...
    def end(self):
        if self.one_step_trained:
            ...
            if multi.is_master():
                ...
                self.__save_sangoi_outputs(save_path)

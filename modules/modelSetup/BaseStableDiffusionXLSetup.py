...
class BaseStableDiffusionXLSetup(
    ...
    def predict(
        ...
        with model.autocast_context:
            ...
            if config.full_vae_mf or config.save_predictions:
                alphas_cumprod = model.noise_scheduler.alphas_cumprod.to(config.train_device)
                sqrt_alpha_prod = alphas_cumprod[timestep] ** 0.5
                sqrt_alpha_prod = sqrt_alpha_prod.flatten().reshape(-1, 1, 1, 1)
                sqrt_one_minus_alpha_prod = (1 - alphas_cumprod[timestep]) ** 0.5
            ...
            if model.tensorboard is not None:
                model.tensorboard.add_scalar(
                    "SangoiW/Timestep",
    ...
    def _decode_latent_sdxl(self, model: StableDiffusionXLModel, latents: Tensor) -> Tensor:
        ...
        with torch.autocast("cuda", dtype=target_dtype):
            return model.vae.decode(latents, return_dict=False)[0]

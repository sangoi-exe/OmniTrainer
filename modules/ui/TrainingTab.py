...
class TrainingTab:
    ...
    def __create_loss_frame(self, master, row, supports_vb_loss: bool = False):
        ...
        loss_modes = [
            str(x)
            for x in list(LossMode)
            if not (self.train_config.model_type.is_flow_matching() and x == LossMode.SANGOI)
        ]
        components.options(frame, row, 1, loss_modes, self.ui_state, "loss_mode_fn")
        row += 1
        ...
        # Huber Delta
        components.label(frame, row, 0, "Huber Delta", tooltip="Delta parameter for huber loss")
        components.entry(frame, row, 1, self.ui_state, "huber_delta", required=True)
        row += 1

        # Sangoi Huber Strength
        components.label(
            frame, row, 0, "Sangoi Huber Strength", tooltip="Additional Sangoi custom Huber loss strength."
        )
        ...
        components.label(
            frame, row, 0, "Sangoi Charbonnier Strength", tooltip="Additional Sangoi custom Charbonnier loss strength."
        )

...
class TrainingTab:
    ...
    def __create_loss_frame(self, master, row, supports_vb_loss: bool = False):
        ...
        # Loss Mode
        components.label(frame, row, 0, "Loss Mode",
                         tooltip="Selects the base loss composition. ORIGINAL keeps standard strengths; SANGOI dynamically weights MSE, MAE, and log-cosh.")
        components.options(frame, row, 1, [str(x) for x in list(LossMode)], self.ui_state, "loss_mode_fn")
        row += 1

        # MSE Strength
        components.label(frame, row, 0, "MSE Strength",
                         tooltip="Mean Squared Error strength for custom loss settings. Strengths should generally sum to 1.")
        components.entry(frame, row, 1, self.ui_state, "mse_strength", required=True)
        row += 1

        # MAE Strength
        components.label(frame, row, 0, "MAE Strength",
                         tooltip="Mean Absolute Error strength for custom loss settings. Strengths should generally sum to 1.")
        components.entry(frame, row, 1, self.ui_state, "mae_strength", required=True)
        row += 1

        # log-cosh Strength
        components.label(frame, row, 0, "log-cosh Strength",
                         tooltip="Log - Hyperbolic cosine Error strength for custom loss settings. Strengths should generally sum to 1.")
        components.entry(frame, row, 1, self.ui_state, "log_cosh_strength", required=True)
        row += 1

        # Huber Strength
        components.label(frame, row, 0, "Huber Strength",
        ...
        components.entry(frame, row, 1, self.ui_state, "huber_strength", required=True)
        row += 1

        # Huber Delta
        components.label(frame, row, 0, "Huber Delta",
                         tooltip="Delta parameter for huber loss")
        components.entry(frame, row, 1, self.ui_state, "huber_delta", required=True)
        row += 1

        # Sangoi Huber Strength
        components.label(frame, row, 0, "Sangoi Huber Strength",
                         tooltip="Additional Sangoi custom Huber loss strength.")
        components.entry(frame, row, 1, self.ui_state, "sangoi_huber_strength", required=True)
        row += 1

        # Sangoi Charbonnier Strength
        components.label(frame, row, 0, "Sangoi Charbonnier Strength",
                         tooltip="Additional Sangoi custom Charbonnier loss strength.")
        components.entry(frame, row, 1, self.ui_state, "sangoi_charbonnier_strength", required=True)
        row += 1

        if supports_vb_loss:
            # VB Strength
            components.label(frame, row, 0, "VB Strength",
                             tooltip="Variational lower-bound strength for custom loss settings. Should be set to 1 for variational diffusion models")
            components.entry(frame, row, 1, self.ui_state, "vb_loss_strength", required=True)
            row += 1

        # Loss Weight function
        components.label(frame, row, 0, "Loss Weight Function",
                         tooltip="Choice of loss weight function. Can help the model learn details more accurately.")
        components.options(frame, row, 1, [str(x) for x in list(LossWeight)
        ...
        if not self.train_config.model_type.is_flow_matching():
            components.label(frame, row, 0, "Gamma",
                             tooltip="Inverse strength of loss weighting. Range: 1-20, applies to Min SNR, P2, and SANGOI.")

...
class AdaptiveDCoef:
    ...
    def scale(self, name: str, step: int, total_steps: int) -> float:
        ...
        if self.debug:
            ...
            print(
                f"[AdaptiveDCoef][DEBUG] scale() name={name}, step={step}, total_steps={total_steps}, conv_step={cs}"
            )
        # --- END: debug entry log ---
    ...
    def scale_vectorized(
        ...
        if self.debug:
            print(
                f"[AdaptiveDCoef][DEBUG] scale_vectorized() names={names}, step={step}, total_steps={total_steps}, device={device}"
            )
        # --- END: debug entry log ---
        ...
        if self.verbose:
            for i, name in enumerate(names):
                print(
                    f"[AdaptiveDCoef] scale_vectorized {name} -> {scales[i].item():.6f}"
                )
        # --- END: verbose summary ---

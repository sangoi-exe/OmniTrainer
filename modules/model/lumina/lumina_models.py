...
try:
    ...
    warnings.warn("Cannot import apex RMSNorm, switch to vanilla implementation", stacklevel=2)
...
class RopeEmbedder:
    def __init__(
        ...
        super().__init__()
        self.theta = theta
        self.axes_dims = [16, 56, 56] if axes_dims is None else axes_dims
        self.axes_lens = [1, 512, 512] if axes_lens is None else axes_lens
...
class NextDiT(nn.Module):
    ...
    def patchify_and_embed(
        ...
        for i, (cap_len, seq_len) in enumerate(zip(l_effective_cap_len, seq_lengths, strict=True)):
...
def NextDiT_2B_GQA_patch2_Adaln_Refiner(params: LuminaParams | None = None, **kwargs):
    if params is None:

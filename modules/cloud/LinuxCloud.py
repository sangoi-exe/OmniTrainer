...
UPSTREAM_ONETRAINER_SOURCE_PATTERN = re.compile(
    r"github\.com[:/]+nerogar/onetrainer(?:\.git)?(?:\b|/|$)",
    re.IGNORECASE,
)


def _uses_upstream_onetrainer_source(value: str) -> bool:
    return bool(UPSTREAM_ONETRAINER_SOURCE_PATTERN.search(value))
...
class LinuxCloud(BaseCloud):
    ...
    def _install_onetrainer(self, update: bool = False):
        ...
        result = self.connection.run(f"test -e {shlex.quote(config.onetrainer_dir)}", warn=True, in_stream=False)
        directory_existed = result.exited == 0
        if not directory_existed:
            if not config.install_onetrainer:
                raise ValueError(
                    f"Cloud OneTrainer directory is missing and automatic install is disabled: {config.onetrainer_dir}"
                )

            install_cmd = config.install_cmd.strip()
            if not install_cmd:
                raise ValueError("Cloud install command is required for this Sangoi checkout")
            if _uses_upstream_onetrainer_source(install_cmd):
                raise ValueError(
                    "Cloud install command points at upstream OneTrainer; configure a command for this Sangoi source"
                )
        ...
        remote_source = self.connection.run(
            f"git -C {shlex.quote(config.onetrainer_dir)} remote -v",
            warn=True,
            hide=True,
            in_stream=False,
        )
        if remote_source.exited == 0 and _uses_upstream_onetrainer_source(remote_source.stdout):
            raise ValueError(
                "Cloud remote OneTrainer directory points at upstream OneTrainer; "
                "configure a Sangoi source checkout before running cloud install/update"
            )
        if remote_source.exited != 0 and directory_existed and update:
            raise ValueError(
                f"Cloud update requires the remote OneTrainer directory to be a git checkout: {config.onetrainer_dir}"
            )

        result = self.connection.run(
            f"test -x {shlex.quote(config.onetrainer_dir)}/.uv/bin/uv "
            f"&& test -x {shlex.quote(config.onetrainer_dir)}/.venv/bin/python",
            warn=True,
            in_stream=False,
        )
        ...
        cmd_env = f"export PATH=$PATH:/usr/local/cuda/bin \
    ...
    def run_trainer(self):
        ...
        cmd = "export PATH=$PATH:/usr/local/cuda/bin \

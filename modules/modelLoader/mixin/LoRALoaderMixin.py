...
class LoRALoaderMixin(metaclass=ABCMeta):
    ...
    @staticmethod
    def scale_lora_state_dict(state_dict: dict, text_encoder_scale: float = 1.0, main_scale: float = 1.0) -> dict:
        scaled_state_dict = {}
        weight_suffixes = (
            ".weight",
            "hada_w1_a",
        ...
        for key, value in state_dict.items():
            ...
            if key.startswith("lora_te"):
                scaled_state_dict[key] = value * text_encoder_scale if is_weight else value
            else:
                scaled_state_dict[key] = value * main_scale if is_weight else value
